...
Doctrine_Manager::connection($db['default']['dsn'], 'main');
...
// *** só usar na hora de gerar o yaml a partir do banco ***
Doctrine_Manager::getInstance()->setAttribute(Doctrine::ATTR_QUOTE_IDENTIFIER, true);

// Require models_path.php
require_once(realpath(dirname(__FILE__) ) . '/models_paths.php');

// load models in subdirectories of models
foreach ($_models_paths as $path)
{
  Doctrine_Core::loadModels(realpath(dirname(__FILE__) . '/..') . DIRECTORY_SEPARATOR . $path."/generated");
  Doctrine_Core::loadModels(realpath(dirname(__FILE__) . '/..') . DIRECTORY_SEPARATOR . $path);
}
